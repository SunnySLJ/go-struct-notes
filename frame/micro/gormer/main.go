...
func main() {
	...
	projectPath = "./"
	modelPath = "internal/template/model"
	daoPath = "internal/template/dao"
	goMod = "go-micro"
	...
	if dsn == "" || projectPath == "" || daoPath == "" || modelPath == "" || goMod == "" {
		fmt.Println("dsn,projectPath,daoPath,modelPath,goMod 为必填参数，请检查")
		os.Exit(1)
	}

	// 创建文件夹（如果已存在会报错，不影响）
	for _, path := range []string{projectPath + daoPath, projectPath + modelPath} {
	...
	for _, table := range tables {
		...
		header := fmt.Sprintf(modelHeader, dirs[len(dirs)-1])
		...
		header = fmt.Sprintf(daoHeader, dirs[len(dirs)-1], goMod, modelPath)
		...
		if _, err = os.Stat(extFile); err != nil {
			...
			_, err = file.WriteString(fmt.Sprintf(daoExtHeader, dirs[len(dirs)-1]) + fmt.Sprintf(daoExtTmpl, structResult.StructName))
			if err != nil {
				file.Close()
	...
	// go fmt files
	exec.Command("go", "fmt", modelPath+"...").Run()
}
